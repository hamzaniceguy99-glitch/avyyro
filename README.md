# Avyyro — running coaching

Static marketing site (HTML / CSS / JS, no build step) hosted on GitHub Pages
at `avyyro.shop`.

> Generated from `C:\Users\souha\coaching-sites-factory` (content file `sites/avyyro.mjs`).
> To change the content, edit that file and run `node build.mjs avyyro` — editing the
...
| 🟠 Important | `contact@avyyro.shop` doesn't exist yet: set up free email forwarding in Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
```
Coaching in running, delivered online: individualized training plans and one-on-one video sessions covering first 5K programs, 10K and half marathon training, marathon preparation, and running form and strength work. Runners follow an 8 to 20 week plan with weekly check-ins, paces calculated for them, and video reviews of their form. This is coaching, not medical care or physical therapy. Services are sold as month-to-month subscriptions from $29 to $159 per month, cancellable at any time. No physical products are sold or shipped. Site: avyyro.shop
```
